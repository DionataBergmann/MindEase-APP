...
export default function ProjectDetailScreen() {
  ...
  const [transitionModal, setTransitionModal] = useState<{ href: string; message: string } | null>(
    null
  );
  ...
  const [projectMenuAnchorLayout, setProjectMenuAnchorLayout] = useState<{
    x: number;
    y: number;
    w: number;
    h: number;
  } | null>(null);
  const [projectMenuOverlayLayout, setProjectMenuOverlayLayout] = useState<{
    x: number;
    y: number;
  } | null>(null);
  ...
  const renderMaterialCard = useCallback(
    (m: Material) => {
      ...
      return (
        <View
          ...
          style={[
            styles.materialCard,
            { backgroundColor: colors.card, borderColor: colors.border, borderWidth: borderW },
          ]}
        >
          <TouchableOpacity
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- id and router are stable
  ...
  const listHeader = (
    <>
      ...
      <View style={styles.buttonsRow}>
        ...
        <Button onPress={handleStudyAll} style={styles.headerBtn}>
          <Feather
            name="bookmark"
            size={18}
            color={colors.primaryForeground}
    ...
    <ThemedView style={styles.container}>
      {materials.length === 0 ? (
        ...
        >
          {listHeader}
          <View
            style={[styles.emptyMaterials, { borderColor: colors.border, borderWidth: borderW }]}
          >
      ...
      {menuOpen && projectMenuAnchorLayout && (
        <View
          ref={projectMenuOverlayRef}
          style={styles.modalOverlay}
          pointerEvents="box-none"
          onLayout={measureProjectMenuOverlay}
        >
          <Pressable style={styles.modalBackdrop} onPress={() => setMenuOpen(false)}>
            {projectMenuOverlayLayout &&
              (() => {
                ...
                const left = Math.min(
                  projectMenuAnchorLayout.x +
                    projectMenuAnchorLayout.w -
                    dropdownWidth -
                    projectMenuOverlayLayout.x,
                  Dimensions.get("window").width - dropdownWidth - projectMenuOverlayLayout.x - 16
                );
                const top =
                  projectMenuAnchorLayout.y +
                  projectMenuAnchorLayout.h -
                  projectMenuOverlayLayout.y;
                return (
                  <View
                    style={[
                ...
                {overlayLayout && (
                  ...
                  >
                    ...
                    <View style={styles.statusSection}>
                      <ThemedText
                        style={[styles.statusSectionLabel, { color: colors.mutedForeground }]}
                      >
                        Mover para
                      </ThemedText>
      ...
      {showEditProject && (
        <View style={styles.modalOverlay} pointerEvents="box-none">
          <Pressable
            ...
            onPress={() => !saving && setShowEditProject(false)}
          >
            <Pressable
              style={[
                styles.modalBox,
      ...
      {showDeleteProject && (
        <View style={styles.modalOverlay} pointerEvents="box-none">
          <Pressable
            style={styles.modalBackdrop}
            onPress={() => !saving && setShowDeleteProject(false)}
          >
            <Pressable
              style={[
                styles.modalBox,
            ...
            >
              ...
              <View style={styles.modalActions}>
                <Button
                  variant="outline"
                  disabled={saving}
                  onPress={() => setShowDeleteProject(false)}
                >
                  <ThemedText style={{ color: colors.primary, fontWeight: "600" }}>
                    Cancelar
                  </ThemedText>
      ...
      {!!deleteMaterialId && (
        <View style={styles.modalOverlay} pointerEvents="box-none">
          <Pressable
            style={styles.modalBackdrop}
            onPress={() => !saving && setDeleteMaterialId(null)}
          >
            <Pressable
              style={[
                styles.modalBox,
            ...
            >
              ...
              <View style={styles.modalActions}>
                <Button
                  variant="outline"
                  disabled={saving}
                  onPress={() => setDeleteMaterialId(null)}
                >
                  <ThemedText style={{ color: colors.primary, fontWeight: "600" }}>
                    Cancelar
                  </ThemedText>
          ...
          <Pressable style={styles.modalBackdrop} onPress={() => setTransitionModal(null)}>
            ...
            >
              ...
              <View style={styles.modalActions}>
                ...
                <Button
                  onPress={() => {
                    router.push(transitionModal.href as any);
